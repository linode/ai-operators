...
def service_with_mock_client():
    with patch('ml_operator.services.kubeflow_pipelines_service.Client') as mock_client_class:
        mock_client = Mock()
...
def test_experiment_creation(service_with_mock_client):
    ...
    mock_client.create_experiment.assert_called_once_with(
        name="test-kb",
        description="ML-Operator experiment for knowledge base test-kb"
    )
...
def test_no_versions_found_error(service_with_mock_client):
    ...
    with pytest.raises(ValueError, match="No versions found for pipeline 'test-pipeline'"):
