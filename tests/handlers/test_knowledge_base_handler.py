...
from ..common import SAMPLE_KB_OBJECT
...
def handler_with_mock_service():
    with patch('ml_operator.handlers.knowledge_base_handler.KubeflowPipelinesService') as mock_service_class:
        mock_service = Mock()
...
async def test_start_pipeline_success(handler_with_mock_service):
    ...
    result = await handler._start_pipeline("test-namespace", "test-kb", SAMPLE_KB_OBJECT, "created")

    assert result == "test-run-id"
    mock_service.run_pipeline.assert_called_once_with("test-namespace", "test-kb", SAMPLE_KB_OBJECT)
...
async def test_start_pipeline_service_failure(handler_with_mock_service):
    ...
    with pytest.raises(Exception, match="Pipeline service failed"):
        await handler._start_pipeline("test-namespace", "test-kb", SAMPLE_KB_OBJECT, "created")
...
async def test_wait_for_completion_success(handler_with_mock_service):
    ...
    result = await handler.wait_for_completion("test-namespace", "test-kb", "test-run-id")
...
async def test_wait_for_completion_failure(handler_with_mock_service):
    handler, mock_service = handler_with_mock_service
    mock_service.wait_for_pipeline_completion.side_effect = Exception("Completion wait failed")
...
async def test_wait_for_completion_timeout(handler_with_mock_service):
    handler, mock_service = handler_with_mock_service
    mock_service.wait_for_pipeline_completion.side_effect = TimeoutError("Pipeline execution timed out")
